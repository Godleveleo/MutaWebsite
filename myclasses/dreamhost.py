...
def moverArchivoProducto(file, ced_identidad):
	if existeArchivo(file)==True:
		fecha = datetime.now()
		...
		UsersMetadata.objects.filter(pk=ced_identidad).update(imagenPerfil=nombre)
...
def existeArchivo(carpeta, archivo):
    try:
        ruta=f"{RUTA}myclasses/assets/upload/{carpeta}/{archivo}"
        fileObj = Path(ruta)
...
def existeArchivoMedia(archivo):
     try:
         ruta=f"{RUTA}myclasses/media/Foto_Perfil/{archivo}"
         fileObj = Path(ruta)
         return fileObj.is_file()
     except Exception as e:
         return False